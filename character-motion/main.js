...
var ship = Crafty.e('2D, Canvas, Color, Solid, Motion, AngularMotion')
            ...
            .bind("KeyDown", function(e){
              ...
              if(e.key === Crafty.keys.UP_ARROW){ /** THRUST **/
                ...
                ship.acceleration().y = ship.rotation > 90 ? 25 : -25 ;
                ship.acceleration().x = (ship.rotation > 90 ? -25 : 25) / (Math.tan((90 - ship.rotation) * (Math.PI / 180)));
                ship.isMoving = true;
              }
            ...
            .bind("EnterFrame", function(e){
              ...
              if(this.x < -40){ this.x = 801; }
              if(this.x > 840){ this.x = -40; }
            });

function slowToStop(){
  console.log("velocity = "+ship.velocity().y);
  ship.acceleration().y = Math.abs(ship.acceleration().y / 2);
  ship.acceleration().x = ship.acceleration().y / (Math.tan((90 - ship.rotation) * (Math.PI / 180)));
  ship.velocity().y = ship.velocity().y / _brakeFactor;
  ship.velocity().x = ship.velocity().x / _brakeFactor;
  if(ship.acceleration().y < 1){
    ship.acceleration().y = 0;
    ship.acceleration().x = 0;
  }
  if(ship.velocity().y > -1 && ship.velocity().y < 1 && ship.acceleration().y === 0){
    ship.velocity().y = 0;
    ship.velocity().x = 0;
